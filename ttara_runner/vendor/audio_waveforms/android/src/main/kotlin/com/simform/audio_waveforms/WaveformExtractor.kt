...
class WaveformExtractor(
    ...
    /**
     * 2026-08-08 patch (vendored fork, see ../../../../../../../PATCH_NOTES.md):
     * [sendProgress] used to call [methodChannel.invokeMethod] on EVERY single data
     * point, each time re-serializing and sending the ENTIRE (ever-growing)
     * [sampleData] list across the platform channel. For large [expectedPoints]
     * (e.g. 150,000, as this app requests for accurate silence detection on long
     * files) that is an O(N^2) cost — confirmed to turn a ~4-8 minute decode into
     * 30+ minutes on real devices. The awaited Dart `extractWaveformData()` Future
     * does NOT depend on this per-point channel call at all (it resolves via
     * [ExtractorCallBack.onProgress] hitting 1.0, called unconditionally above,
     * see [sendProgress] below) — this channel traffic only feeds the
     * [onCurrentExtractedWaveformData] LIVE STREAM, which this app never listens
     * to. Throttling it to at most ~200 sends over the whole extraction preserves
     * that stream's usefulness for any consumer who does want live updates, while
     * eliminating the quadratic blowup for consumers (like this app) who only
     * await the final result.
     */
    private val progressSendInterval: Int by lazy { maxOf(1, expectedPoints / 200) }
    ...
    private fun sendProgress(rms: Float) {
        ...
        // 2026-08-08 patch: throttled — see progressSendInterval doc comment above.
        // currentProgress is a 1-based running count of points sent so far at this point.
        val pointIndex = currentProgress.toInt()
        val isLastPoint = pointIndex >= expectedPoints
        if (pointIndex % progressSendInterval == 0 || isLastPoint) {
