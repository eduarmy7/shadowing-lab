...
public class WaveformExtractor {
    ...
    public func extractWaveform(
        ...
        for i in startIndex..<endIndex {
            ...
            // 2026-08-08 patch (vendored fork): sendWaveformDataToFlutter re-serializes
            // and sends the ENTIRE accumulated waveform array over the platform channel
            // on every point — O(N^2) for large samplesPerPixel. onExtractionComplete
            // (below, after this loop) is what actually resolves the awaited Dart
            // Future and does not depend on this per-point send at all, so it's safe
            // to throttle this to ~200 sends total for consumers of the live stream.
            let pointIndex = i - startIndex + 1
            let sendInterval = max(1, (endIndex - startIndex) / 200)
            let isLastPoint = i == endIndex - 1
            if pointIndex % sendInterval == 0 || isLastPoint {
